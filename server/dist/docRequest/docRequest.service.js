...
exports.DocRequestService = void 0;
...
let DocRequestService = class DocRequestService {
    constructor(DocRequestModel, docApprovalHistModel, documentModal) {
        this.DocRequestModel = DocRequestModel;
        this.docApprovalHistModel = docApprovalHistModel;
        this.documentModal = documentModal;
    }
    async findAll(mode, empl_id) {
        if (mode === "approval") {
            return await this.DocRequestModel.find({})
                .sort({ "requested_on": -1 })
                .exec()
                .then((resultNew) => {
                let approval_list_for_epl = [];
                if (resultNew.length > 0) {
                    resultNew.forEach((req) => {
                        let checkAppor = [];
                        let approvalList = req.approval;
            ...
            requested_doc.forEach((doc) => {
                if (doc._id !== "") {
                    this.documentModal
                        ...
                        if (documenttoEdit.takeout_requested_details &&
                        ...
                        let requestDetails = {
                            takeout_requested_details: Object.assign(Object.assign({}, takeout_request_detail), { current_status: { code: "requested", label: "Requested", request_no: docRequest.request_no }, takeout_request_details_list: takeout_request_details_list }),
                            is_requested_for_takeout: true,
                        };
                        documenttoEdit._doc = Object.assign(Object.assign({}, documenttoEdit._doc), requestDetails);
                        return this.documentModal
                            .findByIdAndUpdate(doc._id, Object.assign({}, documenttoEdit), {
                            new: true,
                        })
                            .then((tre) => {
                        });
    ...
    async update(id, DocRequest, page, updateBy) {
        const { doc_type = "0", requested_doc = [], request_no = "", empl_id = "0", } = DocRequest;
        if (doc_type === "6") {
            requested_doc.forEach((doc) => {
                if (doc._id !== "") {
                    ...
                    this.documentModal
                        ...
                        .then((res) => {
                        let documenttoEdit = res[0];
                        const { takeout_requested_details: { takeout_request_details_list = [], } = {}, takeout_requested_details = {}, } = documenttoEdit;
                        ...
                        if (page === 'issueGenaralIssuance') {
                            requestDetails = {
                                current_status: Object.assign(Object.assign({}, requestDetails.current_status), { takeout_return_date: takeout_return_date, code: "issued", label: "Issued", request_no: request_no }),
                                takeout_request_details_list: takeout_request_details_list,
                            };
                        }
                        else {
                            requestDetails = Object.assign(Object.assign({}, requestDetails.current_status), { current_status: Object.assign(Object.assign({}, requestDetails.current_status), { code: "approved", label: "Approved" }), takeout_request_details_list: takeout_request_details_list });
                        }
            ...
            if (page === "issueGenaralIssuance") {
                const { requested_on = "", requested_doc = [], doc_type = "0", issuance: { doc_issued_by = [] }, doc_requested_department = {}, approval = [] } = DocRequest;
                requested_doc.map((doc) => {
    ...
    async update1(id, DocRequest, page) {
        if (page === "issueGenaralIssuance") {
...
DocRequestService = __decorate([
    common_1.Injectable(),
    __param(0, mongoose_2.InjectModel(docRequest_schema_1.DocRequests.name)),
    __param(1, mongoose_2.InjectModel(docApprovalHistory_schema_1.DocApprovalHistories.name)),
    __param(2, mongoose_2.InjectModel(product_schema_1.Documents.name)),
    __metadata("design:paramtypes", [mongoose_1.Model,
        mongoose_1.Model,
        mongoose_1.Model])
], DocRequestService);
exports.DocRequestService = DocRequestService;
//# sourceMappingURL=docRequest.service.js.map